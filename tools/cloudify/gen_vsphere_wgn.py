...
if __name__=="__main__":
    ...
    tar_path = "$HOME/CloudChef-WorkSpace/cloudchef-vsphere-plugin.tar.gz"
    ...
    tar_cmd = "cd $HOME/CloudChef-WorkSpace/codebase/yacmp-orchestrator ; tar -zcvf " \
              "$HOME/CloudChef-WorkSpace/cloudchef-vsphere-plugin.tar.gz cloudchef-vsphere-plugin/"
    ...
    gen_cmd = "cd /var/cc/plugins ; wagon create -s ./cloudchef-vsphere-plugin.tar.gz"
    ret = ssh.execute(gen_cmd)
    print ret

    current_dir = str(now) + '-vsphere'
    backup_cloudchef = "cd /var/cc/plugins ; mkdir %s ; mv cloudchef* %s" % (current_dir, current_dir)
    ret = ssh.execute(backup_cloudchef)
    print ret

    scp_file = "cd /var/cc/plugins ; cp scp_vsphere_to_server.sh %s" % (current_dir)
    ret = ssh.execute(scp_file)
    print ret

    gen_version = "cd /var/cc/plugins ; echo %s >> vsphere-version.txt" % (now)
